...
"""Independent phase-damping noise channel for VarEFTQC.

This module defines :class:`PhaseDampingChannel`, a convenience wrapper
that applies phase damping independently to one or more wires, each with
...
class PhaseDampingChannel(qml.operation.Operation):
    """Phase damping channel on multiple wires (independent).

    This operation applies :class:`qml.PhaseDamping` to each wire in the
    given list. All wires may share the same damping probability or have
    individual values.
    ...
    def compute_decomposition(wires, gammas):  # pylint: disable=arguments-differ  # noqa
        """Decompose the channel into per-wire PhaseDamping operations.
        ...
        Returns:
            list[qml.operation.Operator]: List of :class:`qml.PhaseDamping`
...
if __name__ == '__main__':
    ...
    def circuit():
        PhaseDampingChannel(wires=[0, 1], gamma=[0.1, 0.2])
        return qml.state()
