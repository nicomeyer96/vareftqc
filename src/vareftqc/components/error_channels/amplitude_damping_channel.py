...
"""Independent amplitude-damping noise channel for VarEFTQC.

This module defines :class:`AmplitudeDampingChannel`, a convenience wrapper
that applies amplitude damping independently to one or more wires, each with
its own damping probability.
"""

import pennylane as qml


class AmplitudeDampingChannel(qml.operation.Operation):
    """Amplitude damping channel on multiple wires (independent).

    This operation applies :class:`qml.AmplitudeDamping` to each wire in
    the given list. All wires may share the same damping probability or
    ...
    def compute_decomposition(wires, gammas):  # pylint: disable=arguments-differ  # noqa
        """Decompose the channel into per-wire AmplitudeDamping operations.
        ...
        Returns:
            list[qml.operation.Operator]: List of :class:`qml.AmplitudeDamping`
...
if __name__ == '__main__':
    ...
    def circuit():
        AmplitudeDampingChannel(gamma=[0.1, 0.2, 0.3], wires=[0, 1, 2])
        return qml.state()
