...
"""Plot encoding and operation losses for Figure 7b.

This script loads the stored experiment logs for a selected Figure 7b run,
...
_RESULTS_DIR = Path(__file__).resolve().parents[1] / "results" / "fig_7b"
...
def plot(pad: float = 0.25, epochs: int = 1000, number: int = 0) -> None:
    """Create and store one Figure 7b plot.
    ...
    axes[1].plot(
        ...
        color="tab:red",
        label=r"$\overline{\mathcal{O}}^{T}_{\mathrm{block}}$",
    )
    axes[1].plot(
        range(len(train_cx_max[: epochs + 1])),
        train_cx_max[: epochs + 1],
        color="tab:orange",
        label=r"$\overline{\mathcal{O}}^{CX}_{\mathrm{block}}$",
    )
    ...
    output_path = _PLOTS_DIR / f"fig_7b.pdf"
